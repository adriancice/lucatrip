...
public interface IEventoRepository extends PagingAndSortingRepository<Evento, Integer> {
	...
	@Transactional
	@Modifying
	@Query(value="delete from eventos where id_evento in(select id_evento from likes where id_like in (select id_like from likes where id_user = :id_user))",  nativeQuery=true)
	public void deleteAllEventosByIdUser(@Param("id_user")int id_user);
